...
# Using ModbusSparseDataBlock with dict-like access (pymodbus 3.12.1 compatible)
...
# Direct access to the datastore's internal simdata for runtime mutation
# (fixes pymodbus 3.13+ deepcopy issue where writes after construction don't reach TCP layer)
_hr_data = _hr.simdata
...
def _get(idx):
    """Read register value using direct block access (bypasses deepcopy trap)."""
    return _hr_data[idx]


def _set(idx, value):
    """Write register value using direct block access (bypasses deepcopy trap)."""
    _hr_data[idx] = int(value) & 0xFFFF
